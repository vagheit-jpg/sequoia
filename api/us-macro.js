...
export default async function handler(req, res) {
  ...
  try {
    ...
    const [
      ...
      umcsR,                                     // FRED 소비자신뢰
      dxyFredR, tnxFredR,                        // FRED DXY + TNX 보완
      ...
      fetchYahooMonthly("UUP",      5),   // 달러인덱스 ETF (점수계산용)
    ...
    const vix    = vixRaw;
    const gld    = gldRaw;
    const dxy    = uupRaw;  // UUP ETF (점수계산용, 기존 유지)

    // 실제 DXY — FRED DTWEXBGS (저장용)
