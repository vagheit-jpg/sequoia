// scripts/fetchMacroCache.js
// GitHub Actions에서 실행: ECOS + Yahoo 데이터 fetch → public/macro-cache.json 저장
...
async function fetchECOS(statCode, itemCode, startDate, endDate, freq) {
...
async function fetchIndexMonthly(yahooTicker) {
  ...
  const url = `https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(yahooTicker)}?interval=1mo&period1=${tenYearsAgo}&period2=${now}&includePrePost=false`;
  const res = await fetch(url, {
    headers: {
  ...
  return timestamps.map((ts, i) => {
    ...
    return { date: `${y}${m}`, price: closes[i] ? Math.round(closes[i]) : null };
...
async function main() {
  ...
  const startDate8 = `${endY - 2}0101`;
  const startDateQ = `${endY - 8}Q1`;

  console.log("Fetching ECOS + Yahoo data...");

  const [gdpR, exportR, rateR, fxR, ppiR, bsiR, cpiR, kospiR, kosdaqR] =
    await Promise.allSettled([
      fetchECOS("200Y102", "10111",   startDateQ, `${endY}Q4`, "Q"),
      fetchECOS("901Y118", "T002",    startDate,  endDate,     "M"),
      fetchECOS("722Y001", "0101000", startDate8, endDate8,    "D"),
      fetchECOS("731Y003", "0000003", startDate8, endDate8,    "D"),
      fetchECOS("404Y014", "*AA",     startDate,  endDate,     "M"),
      fetchECOS("512Y013", "99988",   startDate,  endDate,     "M"),
      fetchECOS("901Y009", "0",       startDate,  endDate,     "M"),
      fetchIndexMonthly("^KS11"),
      fetchIndexMonthly("^KQ11"),
    ]);
  ...
  const indicators = [
    { key:"금리", label:"기준금리",       val:last(rate),         unit:"%",  good:"완화적", warn:"중립", bad:"긴축",   score: score(last(rate),         [4.0,3.0,2.0],  1) },
    { key:"환율", label:"원/달러 환율",   val:last(fx),           unit:"원", good:"강세",   warn:"중립", bad:"약세",   score: score(last(fx),           [1450,1350,1200],1) },
    { key:"GDP",  label:"GDP성장률(YoY)", val:lastYoy(gdp),       unit:"%",  good:"견조",   warn:"완만", bad:"침체",   score: score(lastYoy(gdp),       [-1,1,3],       -1) },
    { key:"PPI",  label:"PPI YoY",        val:lastYoy(ppi),       unit:"%",  good:"안정",   warn:"보통", bad:"원가↑",  score: score(lastYoy(ppi),       [6,3,1],         1) },
    { key:"CPI",  label:"소비자물가YoY",  val:lastYoy(cpi),       unit:"%",  good:"안정",   warn:"보통", bad:"고인플", score: score(lastYoy(cpi),       [5,3,1],         1) },
    { key:"BSI",  label:"BSI 제조업",     val:last(bsi),          unit:"",   good:"확장",   warn:"중립", bad:"수축",   score: score(last(bsi),          [80,90,100],    -1) },
    { key:"수출", label:"수출YoY",        val:lastYoy(exportYoY), unit:"%",  good:"증가",   warn:"보합", bad:"감소",   score: score(lastYoy(exportYoY), [-15,-5,5],     -1) },
  ];
  ...
  const outPath = path.join(__dirname, "../public/macro-cache.json");
  fs.writeFileSync(outPath, JSON.stringify(data));
  console.log(`✅ macro-cache.json saved (${JSON.stringify(data._debug)})`);
}
