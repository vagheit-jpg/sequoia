...
async function getKiwoomToken() {
  ...
  if (!appkey || !secretkey) throw new Error('키움 앱키/시크릿키 누락');
...
export async function fetchKiwoomQuote(symbol: string): Promise<QuoteData> {
  ...
  if (!endpoint || !apiId) throw new Error('KIWOOM_QUOTE_URL 또는 KIWOOM_QUOTE_API_ID 누락');
  ...
  if (!res.ok) throw new Error(`키움 현재가 조회 실패: ${res.status}`);
  const json = await res.json();
  const row = json?.data || json?.output || json;
  ...
  if (!Number.isFinite(price)) throw new Error('키움 응답에서 현재가를 찾지 못했습니다.');
