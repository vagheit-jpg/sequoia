  ...
  return (
    ...
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap', marginBottom: 12 }}>
        ...
        <div style={{ color: theme.muted, fontSize: 12, alignSelf: 'center' }}>기존 그래프 감성을 최대한 살린 Lite 메인 차트</div>
      </div>
      ...
      <div style={{ width: '100%', height: 'min(52vw, 420px)', minHeight: 290 }}>
        <ResponsiveContainer>
          <LineChart data={data} margin={{ top: 8, right: 10, left: -16, bottom: 8 }}>
            ...
            <YAxis tick={{ fill: theme.muted, fontSize: 11 }} axisLine={false} tickLine={false} width={76} tickFormatter={(value) => `${Math.round(value / 1000)}k`} />
            ...
            <Line type="monotone" dataKey="perLow" name="PER Low" stroke={theme.green} strokeWidth={1.3} strokeDasharray="6 4" dot={false} />
            <Line type="monotone" dataKey="perMid" name="PER Mid" stroke={theme.green} strokeWidth={1.1} strokeDasharray="2 4" dot={false} />
            <Line type="monotone" dataKey="perHigh" name="PER High" stroke={theme.green} strokeWidth={1.3} strokeDasharray="6 4" dot={false} />
            <Line type="monotone" dataKey="pbrLow" name="PBR Low" stroke={theme.orange} strokeWidth={1.2} strokeDasharray="6 4" dot={false} />
            <Line type="monotone" dataKey="pbrMid" name="PBR Mid" stroke={theme.orange} strokeWidth={1.0} strokeDasharray="2 4" dot={false} />
            <Line type="monotone" dataKey="pbrHigh" name="PBR High" stroke={theme.orange} strokeWidth={1.2} strokeDasharray="6 4" dot={false} />
          </LineChart>
        </ResponsiveContainer>
      </div>
    </section>
